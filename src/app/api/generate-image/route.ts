...
import { uploadBufferImageKit } from "@/lib/imagekit";
...
// inferImageSize reads width and height from the uploaded image (via sharp), computes aspect ratio,and returns one of the allowed `size` values for OpenAI image edits.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await Sentry.startSpan(
      {
        name: `invoke_agent image_generation`,
        op: "gen_ai.invoke_agent",
        attributes: {
          "gen_ai.agent.name": "image_generation",
        ...
        return await Sentry.startSpan(
          ...
          async (span) => {
            const out = await generateImage({
              model: openaiProvider!.imageModel(model as any),
              prompt: {
                images: [imageBuffer],
                text: prompt,
              },
    ...
    const { url: resultImageUrl } = await uploadBufferImageKit({
      buffer: resultBuffer,
